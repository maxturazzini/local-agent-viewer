...
- **Fix** (`lav/parsers/jsonl.py`, `lav/queries.py`):
  ...
  - **acompact checkpoints are not subagents** (followup): auto-compaction files `agent-acompact-<hex>.jsonl` reuse the parent's sessionId and carry `agentId="acompact-…"` + `isSidechain=true`, but only DUPLICATE a slice of the parent's own messages (same uuids). The initial rekey treated them as subagents → phantom child conversations that double-counted the parent's messages/tokens (measured: 5299 duplicated messages across 169 phantom children on macChia). Parser now skips `agent-acompact-*` files entirely; `purge_acompact_children()` runs each parse to sweep pre-fix phantoms; `ingest_remote_sessions` drops any `::agent-acompact-` bundle defensively. Verified on AIMaxViewer: phantom children 2→0, duplicates 390→0, the 60 real subagents untouched, incremental stays clean.
