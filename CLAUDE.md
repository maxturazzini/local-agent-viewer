...
lav backfill codex-titles     # rebuild Codex display/summary from state_<N>.sqlite (LAV-91)
lav backfill codex-titles --state-db /tmp/agent_state_5.sqlite  # apply another node's titles

# Entity graph (LAV-93) — local DB only, run on the node that classifies
lav entities build --since 2026-09-14   # mentions from people/clients, involvement found in messages
lav entities import seed.json           # PRIVATE seed: roles, works_for, aliases (never commit it)
lav entities list --kind person --at 2026-09-15   # role at a date, counted once per root session
lav entities pending                    # proposals and mentioned entities with no role
lav entities confirm 42 --valid-from 2025-09-03   # or --reject / --valid-to to close a period
...
**Error taxonomy** (LAV-85): `is_error = 1` ≠ "the tool ran and failed". **27,4%** of bash errors never executed — `denied` (permission/blocked), `sibling` (a parallel call in the same block failed), `interrupted`, `rejected` (malformed call). Real Bash rate **4,21%**, raw 5,71%. `queries._error_class_expr()` is the single classifier for all six tables; class names are Python constants that GENERATE the SQL so nothing can drift. `ERROR_CLASSES_NEVER_EXECUTED` must leave BOTH numerator and denominator — a call that never ran can neither succeed nor fail. All six tables now return `errors/measured/error_rate/never_executed/errors_unclassified`. **`errors_unclassified` is mandatory reading**: between the migration and `lav backfill tool-outcomes` the `error_text` column exists but is empty, so `never_executed` reads 0 when the truth is "not looked yet". Markers are TUNED against the corpus (filter class=`failed` AND `exit_code IS NULL` and read the residual — currently 0,8%), never guessed. `backfill.CHILD_TABLES` vs `OUTCOME_COLUMNS` is now enforced at import by `_assert_outcome_columns_shipped()`.

**Subagent spawning** (LAV-82): the spawn tool is `Task` (≤2026-02-27) **or** `Agent` (≥2026-03-01) **or** `Workflow` — matching only `"Task"` is what froze `subagent_invocations` in March. Never gate on `subagent_type`: 109/1.217 `Agent` calls and 100% of `Workflow` calls have none. `tool_outcomes.subagent_row_from_tool_input()` is the ONE ladder, called by both the parser and `tool_row_matches()` — a second copy makes `stamp_tool_call_id()` claim the wrong row. New columns `subagent_invocations.spawn_tool`/`.workflow_id` + `interactions.workflow_id`, all OUT of the UNIQUE key (attributes, not identity). `workflow_id` = the `wf_<id>` cohort: from the transcript path for child sessions (`<parent>/subagents/workflows/wf_<id>/agent-*.jsonl`, no `--since` needed — agent files bypass both gates), and from the **tool_result** for the parent invocation (`Transcript dir: …`), recovered on both nodes by `lav backfill tool-outcomes` with no reparse. Cohorts are NOT inferable from timestamps: 21/43 parent sessions ran more than one workflow. `spawn_tool` seed is one-shot (`parse_state` flag + date guard `SPAWN_TOOL_RENAME_DATE`).
...
**Codex titles** (LAV-91): Codex injects its context as `role: "user"` messages, so the first-message heuristic titled **90%** of Codex interactions with a wrapper (`<permissions instructions>` alone was 190 of 249 rows). The title is **NOT in the rollout JSONL** — every event type was checked. It lives in `~/.codex/state_<N>.sqlite`, table `threads`, the same store the app-server's official `thread/list` reads. Canonical column order is `name` (the real title, has a `thread/setName` method, but OPTIONAL — 11/290 threads) then `preview` (declared `required` in the protocol, always present, first user message already cleaned). The table's own `title` column is NOT read: it is not in the protocol and duplicates `preview`. `config.get_codex_state_db()` globs `state_*.sqlite` and takes the **highest generation** — the `<N>` is the file generation, not a revision counter (`state_5` carries 52 rows in its own `_sqlx_migrations`; `state_1..4` are deleted), so a hardcoded number silently reads a stale DB after an upgrade. Read the FILE, not the app-server: on prod-host the `codex` command does not exist (Desktop only). Every failure path (missing, locked, restructured) warns and returns `{}` → titles fall back to message text; never `immutable=1`, which would skip the `-wal` Codex is still writing. `_CODEX_SYSTEM_TAG_NAMES` is **tuned against the corpus, never guessed**: `collaboration_mode`/`apps_instructions`/`plugins_instructions` are invisible until the tag before them is removed from the same message, and skipping that iteration leaves 148/235 rows dirty instead of 1. `lav backfill codex-titles` reports the residual as `injected_titles_after` — mandatory reading after any edit to the list. Two blocks (`# AGENTS.md instructions`, `Thread coordination:`) carry no tag and are matched by prefix BEFORE the `"<" not in text` fast path. Codex titles that are themselves wrapped (`<task>`, `<realtime_delegation>`: sub-agent delegations) are **unwrapped, not stripped** — the useful text is inside. **Run the backfill on every node**, and note the asymmetry: Codex's state DB never leaves its host, so 0 of 235 agent sessions resolve against the collector's copy — copy the agent's `state_<N>.sqlite` over and pass `--state-db`. Unrelated but visible in the data: 48/235 Codex "conversations" are automatic permission checks, marked by Codex in `threads.source` as `{"subagent":{"other":"guardian"}}`.

**Entity graph** (LAV-93): `interaction_metadata.people/clients` are per-interaction tags; `lav/entities.py` turns them into three tables created by `init_db()` on every node — `entities` (person | organization | self, `name_key` merges spellings), `entity_edges` (**always dated**: `valid_from` NULL = start unknown, `valid_to` NULL = still valid; `alias_of`, `works_for`, and the roles `client|prospect|partner|supplier|own|role_unknown` pointing at `self`), `entity_mentions` (entity → interaction, dated by first/last message, with `root_session_id` and `involvement`). **Roles are never asked to the model**: they come from a private seed (`lav entities import`, format `docs/entities.seed.example.json`) or `lav entities confirm`; rules only propose (`confirmed = 0`), only confirmed `alias_of` merges names and only confirmed role edges give a role (`works_for` may stay a proposal). **Involvement is deterministic**, found in the messages via `classifiers/sources.segments()`: `conversation` > `data` (communication tool results) > `listing` (automated runs, taxonomy `entities.automated_prompt_prefixes`) > `instructions` (skill body only) > `tool_output` > `not_found`. `lav entities list` counts ROOT sessions (subagents once) and by default only `conversation`/`data`, plus `listing` for entities with a known role. Derived from `interaction_metadata`, never synced: build it where classification runs, and rebuild after a reclassification (idempotent per interaction). Details: `docs/DATA_MODEL.md` §7.
...
- `LAV_CLASSIFY_MAX_CHARS` — max chars of interaction text sent to the model (default: `12000`; prod `50000`, keep it under ~70000: the Foundry deployment rejects single requests above ~22k tokens, LAV-92)
...
- `LAV_FOUNDRY_TEMPERATURE` — float, sent on every Foundry attempt when set (unset = endpoint default). Prod: `0`: without it the same input gave different people/clients on 7 of 29 sessions (LAV-92)
...
- **Deploy decision tree** — branch on what changed in the diff (full table with commands: [docs/infrastructure.md](docs/infrastructure.md#deploy-decision-tree)): static-only → `git pull` + browser refresh, no restart; `pyproject.toml` → also `pip install -e .`; any `lav/*.py` → also restart the server python process, and **verify the pid actually changed** (`ps -o pid,lstart -p <pid>`): the two nodes launch it differently, so a single pgrep pattern silently misses one of them and you believe you restarted when you did not. Match on `lav-venv/bin/lav-server` for a console-script launch and on `python.*-m lav.server` for the wrapper-script launch — check both; `lav/mcp_server.py` → also restart `lav-mcp` (drops live MCP clients); version bump → tag the release.
